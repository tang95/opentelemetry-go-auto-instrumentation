...
var imrocHttpClientInstrumenter = BuildImrocHttpClientOtelInstrumenter()

func clientOnEnter(call api.CallContext, t *req.Transport, req *http.Request) {
	...
	ctx := imrocHttpClientInstrumenter.Start(req.Context(), netHttpRequest)
...
func clientOnExit(call api.CallContext, res *http.Response, err error) {
	...
	if res != nil {
		imrocHttpClientInstrumenter.End(ctx, &netHttpRequest{
		...
		imrocHttpClientInstrumenter.End(ctx, &netHttpRequest{}, &netHttpResponse{
			statusCode: 500,
		}, err)
	}
}
